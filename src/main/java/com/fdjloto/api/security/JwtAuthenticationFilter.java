...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    //     if (token != null && jwtUtils.validateJwtToken(token)) {
    //         String username = jwtUtils.getUserFromJwtToken(token);
    //         UUID userId = jwtUtils.extractUserId(token); // ✅ Extraction sécurisée de `userId`
    ...
    //         // 🔥 Création de l'objet `Authentication` avec userId dans les détails
    //         UsernamePasswordAuthenticationToken authentication =
    //                 new UsernamePasswordAuthenticationToken(username, null, authorities);
    //         authentication.setDetails(userId); // ✅ Ajout de `userId` dans le contexte de sécurité
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (token != null && jwtUtils.validateJwtToken(token)) {
            String username = jwtUtils.getUserFromJwtToken(token);
            // UUID userId = jwtUtils.extractUserId(token); // ✅ Extraction de l'UUID utilisateur
            UUID userId = UUID.fromString(jwtUtils.extractUserId(token)); // ✅ Conversion sécurisée
            ...
            UsernamePasswordAuthenticationToken authentication =
                    new UsernamePasswordAuthenticationToken(username, null, authorities);
            authentication.setDetails(userId); // ✅ Ajout de userId dans le contexte
